...
本批发布快照包含 73 篇：`collectedCount: 69`、`reviewedCount: 0`，另有 4 篇保留演示标识的原有答案。后续同步需等对应 GitHub Actions 部署完成后，公开页面才会更新。

这批内容后续随 `npm run sync:pages` 保留或更新，无需逐篇点击「通过并发布」。导出仍读取本机运行库，因此须先按 [内容维护说明](handbook-maintenance.md#在现有平台中查看) 导入本批原始内容。运行库中的初稿状态与 Pages 的资料收录状态分别保留；Pages 收录不会把 69 篇直接写成 Node 读者页的已发布版本。
...
4. 命令只读当前本地演示运行库，导出已审核公开内容及显式收录的资料初版，校验并构建静态站点，再将快照作为独立提交推送至 `origin/main`。GitHub Actions 随后运行检查并部署 Pages；对应提交的 `build`、`deploy` 均成功后，刷新公开网站核对正文与版本。
...
`export:pages` 默认读取本地演示库，也可显式传 `--demo`；`--production` 选择独立生产运行库。导出文件仍固定为 `community/pages-reviewed.json`，构建输出为 `community/pages-dist`。文件名为兼容既有同步流程而保留，包含资料收录内容时使用 `public-guide` 模式。导出和同步不代替人工核验；除显式保留的演示卡外，只有已审核公开内容或明确收录清单中的资料版本能够进入快照。
...
`community/pages-reviewed.json` 使用 `mode: public-guide`，包括公开正文、逐句引用、主题与适用范围、资料读取和核验说明、生成时间及内容摘要校验值。资料初版标记为 `collected`，与 `approved` 的人工审核内容分开；导出结果分别报告 `collectedCount` 和 `reviewedCount`。已人工确认的 AI 辅助内容保留原始来源类型，读者详情明确显示相应审核说明。原有演示答案继续标注「演示内容」。仅保存过旧版「通过」意见，不会自动变成人工审核公开版。
