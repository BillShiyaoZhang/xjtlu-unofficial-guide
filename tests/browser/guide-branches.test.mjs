...
test('local reader branches preserve independent topic statements, search context and historical versions', async t => {
  ...
  for (const viewport of [{ width: 1440, height: 1040 }, { width: 390, height: 844 }]) {
    ...
    await page.locator('#branches-view').click();
    ...
    await page.locator('#answer-branches [data-answer-id="' + firstId + '"]').waitFor();
    assert.equal(await page.locator('#search [name=query]').inputValue(), 'SYNTHETIC_BRANCH_ALPHA');
    assert.equal(await page.locator('#branches-view').getAttribute('aria-pressed'), 'true');
    await page.reload();
    await page.locator('#answer-branches [data-answer-id="' + firstId + '"]').waitFor();
    assert.equal(await page.locator('#search [name=query]').inputValue(), 'SYNTHETIC_BRANCH_ALPHA');
    await page.locator('#search [name=query]').fill('SYNTHETIC_NO_MATCH');
    await page.locator('#search button').click();
    await page.locator('#answer-empty').waitFor({ state: 'visible' });
    await page.locator('#empty-reset').click();
    await page.locator('#answer-empty').waitFor({ state: 'hidden' });
    assert.equal(await page.locator('#search [name=query]').inputValue(), '');
    assert.equal(new URL(page.url()).searchParams.has('query'), false);
    await page.locator('#search [name=query]').fill('SYNTHETIC_UNSUBMITTED_NO_MATCH');
    await page.locator('#list-view').click();
    await page.locator('#answer-empty').waitFor({ state: 'visible' });
    assert.equal(new URL(page.url()).searchParams.get('query'), 'SYNTHETIC_UNSUBMITTED_NO_MATCH');
    await page.reload();
    await page.locator('#answer-empty').waitFor({ state: 'visible' });
    assert.equal(await page.locator('#list-view').getAttribute('aria-pressed'), 'true');
    assert.equal(await page.locator('#search [name=query]').inputValue(), 'SYNTHETIC_UNSUBMITTED_NO_MATCH');
    await page.locator('#search [name=query]').fill('SYNTHETIC_BRANCH_ALPHA');
    await page.locator('#reader-topics button[data-topic="' + first.topic.id + '"]').click();
    await page.locator('#answer-list h3').filter({ hasText: current.data.title }).waitFor();
    assert.equal(await page.locator('#answer-list .answer-item').count(), 1, 'a topic shortcut applies the pending keyword before displaying results');
    assert.equal(new URL(page.url()).searchParams.get('query'), 'SYNTHETIC_BRANCH_ALPHA');
    await page.locator('#search [name=query]').fill('SYNTHETIC_ANOTHER_NO_MATCH');
    await page.locator('#topic').selectOption('');
    await page.locator('#answer-empty').waitFor({ state: 'visible' });
    assert.equal(new URL(page.url()).searchParams.get('query'), 'SYNTHETIC_ANOTHER_NO_MATCH');
    assert.equal(await page.locator('#answer-list .answer-item').count(), 0, 'the topic select also applies a pending keyword');
