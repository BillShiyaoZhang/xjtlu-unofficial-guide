...
import { recordAnswerShare } from '@/lib/measurement';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await enforceRateLimit(request, 'answer-share', 60, 600);
    const body = await readJsonBody(request, 4_000);
    const recorded = await recordAnswerShare({
      queryEventId:
    ...
    return noStoreJson({ data: { recorded }, request_id: requestId });
  } catch (error) {
