...
PWA 不会离线缓存答案、举报、研究线索或编辑内容。生产构建中的 Service Worker 只缓存静态断网页；网络失败时明确提示重新联网，避免把过期校园信息伪装成当前答案。开发模式不会注册 Service Worker。
...
在支持的浏览器中可以把公开端添加到手机主屏幕。真实手机安装测试需要 HTTPS 安全上下文；本地 `localhost` 可用于桌面安装性验证。本项目没有因此部署到 OpenAI Sites。
